...
bool isScriptThreaded(int scriptID)
{
    ...
    if (-1!=simGetScriptProperty(scriptID,&prop,&obj))
        return((prop&sim_scripttype_threaded_old)!=0);
    return(false);
}
...
{ // This is called just once, at the start of CoppeliaSim
    ...
    if (simLib==NULL)
    {
        printf("simExtLuaRemoteApiClient: error: could not find or correctly load the CoppeliaSim library. Cannot start the plugin.\n"); // cannot use simAddLog here.
        return(0); // Means error, CoppeliaSim will unload this plugin
    }
    if (getSimProcAddresses(simLib)==0)
    {
        printf("simExtLuaRemoteApiClient: error: could not find all required functions in the CoppeliaSim library. Cannot start the plugin.\n"); // cannot use simAddLog here.
